...
author: "Miles Dosman Banks, B00965951"
date: "28/01/2025"
output:
...
-->
There are three gneeral clumps of points, with many disjoint points throughout the graph.
...
```{r}
p <- ggplot(data = penguins,mapping = aes(x=bill_length_mm, y=bill_depth_mm)) +geom_point()
...
-->
Adding color reveals that the three clumps all belong to different species, with some overlap between them.
...
```{r}
p <- ggplot(data = penguins,mapping = aes(x=bill_length_mm, y=bill_depth_mm)) +geom_point()
p + geom_point(aes(color=species)) + ggtitle("Penguin Bill Size in Different Species") + xlab("Bill Length") + ylab("Bill Depth")
```
...
```{r}
p <- ggplot(data = penguins,mapping = aes(x=flipper_length_mm, y=body_mass_g)) +geom_point()
p + geom_point(aes(color=species)) + ggtitle("Penguin Flipper Length and Body Mass Between Species") + xlab("Flipper Length") + ylab("Body Mass")
```
